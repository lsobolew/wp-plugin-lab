...
6. **Never change a static block's `save()` without adding a `deprecated` entry.** The markup is
   already in people's databases; without the entry every existing post shows the block as invalid.
   `docs/blocks.md` explains the rule, and an e2e test enforces it.
7. **Blocks are TypeScript, built with Vite** (`./bin/wpx dev` to watch). Do not reintroduce
...
plugins/my-plugin/       free edition (WordPress.org)
  blocks/                block sources in TypeScript
...
./bin/wpx test e2e --targets=latest --edition=both   # sweeps the themes
```
